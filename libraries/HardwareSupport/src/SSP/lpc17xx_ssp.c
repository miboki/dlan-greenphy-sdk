...
#define GREEN_PHY_SSP_CS_SSP0_PORT 0
#define GREEN_PHY_SSP_CS_SSP0_PIN  16
#define GREEN_PHY_SSP_CS_SSP1_PORT 0
#define GREEN_PHY_SSP_CS_SSP1_PIN  6
...
void SSP_Init(LPC_SSP_TypeDef *SSPx, SSP_CFG_Type *SSP_ConfigStruct)
{
	...
	if(SSPx == LPC_SSP0) {
		...
		/* SPI basic configuration Pins */
    	LPC_PINCON->PINSEL0 |= 0x2<<30; //SCK0
    	LPC_PINCON->PINSEL1 |= 0x2<<2;  //MISO0
    	LPC_PINCON->PINSEL1 |= 0x2<<4;	//MOSI0

    	LPC_PINCON->PINMODE0 |= 0x2<<30; //SCK0
    	LPC_PINCON->PINMODE1 |= 0x2<<2;  //MISO0
    	LPC_PINCON->PINMODE1 |= 0x2<<4;	//MOSI0

    	GPIO_SetDir(GREEN_PHY_SSP_CS_SSP0_PORT,GPIO_MAP_PIN(GREEN_PHY_SSP_CS_SSP0_PIN),GPIO_OUT);
    	GPIO_SetValue(GREEN_PHY_SSP_CS_SSP0_PORT,GPIO_MAP_PIN(GREEN_PHY_SSP_CS_SSP0_PIN));
    	...
    	LPC_PINCON->PINMODE0 |= 0x2<<14; //SCK1
    	LPC_PINCON->PINMODE0 |= 0x2<<16; //MISO1
    	LPC_PINCON->PINMODE0 |= 0x2<<18; //MOSI1

    	GPIO_SetDir(GREEN_PHY_SSP_CS_SSP1_PORT,GPIO_MAP_PIN(GREEN_PHY_SSP_CS_SSP1_PIN),GPIO_OUT);
    	GPIO_SetValue(GREEN_PHY_SSP_CS_SSP1_PORT,GPIO_MAP_PIN(GREEN_PHY_SSP_CS_SSP1_PIN));
...
void SSP_Init_UV(LPC_SSP_TypeDef *SSPx, SSP_CFG_Type *SSP_ConfigStruct)
{
	uint32_t tmp;
...
* @return		None
*******************************************************************************/
void SSP_ConfigStructInit_UV(SSP_CFG_Type *SSP_InitStruct)
{
	/* use these settings for the QCA7k */
	SSP_InitStruct->CPHA = SSP_CPHA_FIRST;
	SSP_InitStruct->CPOL = SSP_CPOL_HI;
	SSP_InitStruct->ClockRate = 1600000;
...
/*********************************************************************//**
 * @brief		SSP CS manual set
 * @param[in]	SSPx	SSP peripheral selected, should be
 * 						- LPC_SSP0: SSP0 peripheral
 * 						- LPC_SSP1: SSP1 peripheral
 * @param[in]	toggle	1 is high, 0 is low
 * @return 		Actual status of SSP CS
 **********************************************************************/
int SSP_SSELToggle(LPC_SSP_TypeDef* SSPx, uint32_t toggle)
{
	int rv = 0;
	CHECK_PARAM(PARAM_SSPx(SSPx));
	uint32_t pin = 0;
	...
	if(SSPx == LPC_SSP0)
	{
		pin = (0x1<<GREEN_PHY_SSP_CS_SSP0_PIN);
	}
	else if(SSPx == LPC_SSP1)
	{
		pin = (0x1<<GREEN_PHY_SSP_CS_SSP1_PIN);
	}

	if (pin)
	{
		rv = LPC_GPIO0->FIOPIN & pin;
						...
						SSP_TRANSFER_Type xfType)
{
	...
	// DMA mode ----------------------------------------------------------------------
	else if (xfType == SSP_TRANSFER_DMA){
		if(dataCfg->rx_data)
		{
			LPC_SSP0->DMACR = 0;

			DMAChannel_Init( 1, SM2P , NULL, dataCfg->length);
			DMAChannel_Init( 0, P2M , dataCfg->rx_data, dataCfg->length);

			LPC_SSP0->DMACR = 0x3;
			DMAChannel_TransferComplete(1, portMAX_DELAY);
			DMAChannel_TransferComplete(0, portMAX_DELAY);
		}
		else
		{
			DMAChannel_Init( 1, M2P , dataCfg->tx_data, dataCfg->length);
			DMAChannel_TransferComplete(1, portMAX_DELAY);
		}
