...
static int sendPacket(MQTTClient* c, int length, Timer* timer)
{
    ...
    if (sent == length)
    {
        TimerCountdown(&c->last_sent, c->keepAliveInterval); // record the fact that we have SUCCESS_MQTTfully sent the packet
        rc = SUCCESS_MQTT;
    }
    else
        rc = FAILURE;
...
static int readPacket(MQTTClient* c, Timer* timer)
{
    ...
    if (c->keepAliveInterval > 0)
        TimerCountdown(&c->last_received, c->keepAliveInterval); // record the fact that we have SUCCESS_MQTTfully received a packet
exit:
    return rc;
}
