...
BaseType_t xInitMqtt()
{
	...
	if( xMqttTaskHandle == NULL )
	{
		...
		connectData.clientID.cstring = netconfigMQTT_CLIENT;
		connectData.username.cstring = netconfigMQTT_USER;
		connectData.password.cstring = netconfigMQTT_PASSWORT;
		connectData.willFlag = netconfigMQTT_USEWILL;
		if( connectData.willFlag != 0 ){
			connectData.will.qos = netconfigMQTT_WILL_QOS;
			connectData.will.retained = netconfigMQTT_WILL_RETAIN;
			connectData.will.message.cstring = netconfigMQTT_WILL_TOPIC;
			connectData.will.topicName.cstring = netconfigMQTT_WILL_MESSAGE;
		}

		if( ( iStatus = MQTTConnect(&xClient, &connectData)) != 0 ){
